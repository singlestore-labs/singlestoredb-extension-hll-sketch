...
HllArray<A>* HllArray<A>::newHll(const void* bytes, size_t len, const A& allocator) {
  if (len < hll_constants::HLL_BYTE_ARR_START) {
    THROW_OUT_OF_RANGE("Input data length insufficient to hold HLL array");
  }
  ...
  if (data[hll_constants::FAMILY_BYTE] != hll_constants::FAMILY_ID) {
    THROW_INVALID_ARG("Input array is not an HLL sketch");
  }

  const hll_mode mode = HllSketchImpl<A>::extractCurMode(data[hll_constants::MODE_BYTE]);
  if (mode != HLL) {
    THROW_INVALID_ARG("Calling HLL array constructor with non-HLL mode data");
  }
  ...
  if (len < static_cast<size_t>(hll_constants::HLL_BYTE_ARR_START + arrayBytes)) {
    THROW_OUT_OF_RANGE("Input array too small to hold sketch image");
  }
...
HllArray<A>* HllArray<A>::newHll(std::istream& is, const A& allocator) {
  ...
  if (mode != HLL) {
    THROW_INVALID_ARG("Calling HLL construtor with non-HLL mode data");
  }
...
template<typename A>
bool HllArray<A>::serialize2(bool compact, unsigned char** outBytes, size_t* outSize) const {
