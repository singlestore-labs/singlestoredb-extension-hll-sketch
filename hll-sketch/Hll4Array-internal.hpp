...
void Hll4Array<A>::shiftToBiggerCurMin() {
  ...
  for (uint32_t i = 0; i < configK; i++) { //724
    ...
    if (oldStoredValue == 0) {
      THROW_RUNTIME_ERR("Array slots cannot be 0 at this point.");
    }
    if (oldStoredValue < hll_constants::AUX_TOKEN) {
      ...
      numAuxTokens++;
      if (auxHashMap_ == nullptr) {
        THROW_LOGIC_ERR("auxHashMap cannot be null at this point");
      }
    }
  }
  ...
  if (auxHashMap_ != nullptr) {
    ...
    for (const auto coupon: *auxHashMap_) {
      ...
      if (newShiftedVal < hll_constants::AUX_TOKEN) { // 756
        if (newShiftedVal != 14) {
          THROW_LOGIC_ERR("newShiftedVal != 14 for item in old auxHashMap despite curMin increment");
        }
    ...
    if (numAuxTokens != 0) {
      THROW_LOGIC_ERR("No auxiliary hash map, but numAuxTokens != 0");
    }
  }

  if (newAuxMap != nullptr) {
    if (newAuxMap->getAuxCount() != numAuxTokens) {
      THROW_RUNTIME_ERR("Inconsistent counts: auxCount: " + std::to_string(newAuxMap->getAuxCount())
