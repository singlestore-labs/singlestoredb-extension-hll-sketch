...
CouponList<A>* CouponList<A>::newList(const void* bytes, size_t len, const A& allocator) {
  if (len < hll_constants::LIST_INT_ARR_START) {
    THROW_OUT_OF_RANGE("Input data length insufficient to hold CouponHashSet");
  }

  const uint8_t* data = static_cast<const uint8_t*>(bytes);
  if (data[hll_constants::PREAMBLE_INTS_BYTE] != hll_constants::LIST_PREINTS) {
    THROW_INVALID_ARG("Incorrect number of preInts in input stream");
  }
  if (data[hll_constants::SER_VER_BYTE] != hll_constants::SER_VER) {
    THROW_INVALID_ARG("Wrong ser ver in input stream");
  }
  if (data[hll_constants::FAMILY_BYTE] != hll_constants::FAMILY_ID) {
    THROW_INVALID_ARG("Input stream is not an HLL sketch");
  }

  hll_mode mode = HllSketchImpl<A>::extractCurMode(data[hll_constants::MODE_BYTE]);
  if (mode != LIST) {
    THROW_INVALID_ARG("Calling list constructor with non-list mode data");
  }
  ...
  if (len < expectedLength) {
    THROW_OUT_OF_RANGE("Byte array too short for sketch. Expected " + std::to_string(expectedLength)
...
CouponList<A>* CouponList<A>::newList(std::istream& is, const A& allocator) {
  ...
  if (!is.good())
    THROW_RUNTIME_ERR("error reading from std::istream"); 
...
vector_u8<A> CouponList<A>::serialize(bool compact, unsigned header_size_bytes) const {
  ...
  switch (sw) {
    ...
    default:
      THROW_RUNTIME_ERR("Impossible condition when serializing");
  }
...
template<typename A>
bool CouponList<A>::serialize2(bool compact, unsigned char** outBytes, size_t* outSize) const {
  unsigned header_size_bytes = 0;
  const size_t sketchSizeBytes = (compact ? getCompactSerializationBytes() : getUpdatableSerializationBytes()) + header_size_bytes;
  *outBytes = (unsigned char*) malloc(sketchSizeBytes);
  if (!*outBytes)
  {
      return false;
  }
  *outSize = sketchSizeBytes;
  uint8_t* bytes = *outBytes + header_size_bytes;
...
HllSketchImpl<A>* CouponList<A>::couponUpdate(uint32_t coupon) {
  ...
  THROW_RUNTIME_ERR("Array invalid: no empties and no duplicates");
}
