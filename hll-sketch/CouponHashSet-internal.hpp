...
{
  if (lgConfigK <= 7) {
    THROW_INVALID_ARG("CouponHashSet must be initialized with lgConfigK > 7. Found: "
...
CouponHashSet<A>* CouponHashSet<A>::newSet(const void* bytes, size_t len, const A& allocator) {
  ...
  if (mode != SET) {
    THROW_INVALID_ARG("Calling set constructor with non-set mode data");
  }
  ...
  if (lgK <= 7) {
    THROW_INVALID_ARG("Attempt to deserialize invalid CouponHashSet with lgConfigK <= 7. Found: "
                                + std::to_string(lgK));
  }   
...
void CouponHashSet<A>::growHashSet(uint8_t tgtLgCoupArrSize) {
  ...
  for (uint32_t i = 0; i < srcLen; ++i) { // scan existing array for non-zero values
    ...
    if (fetched != hll_constants::EMPTY) {
      ...
      THROW_RUNTIME_ERR("Error: Found duplicate coupon");
    }
  }
...
static int32_t find(const uint32_t* array, uint8_t lgArrInts, uint32_t coupon) {
  ...
  THROW_INVALID_ARG("Key not found and no empty slots!");
}
