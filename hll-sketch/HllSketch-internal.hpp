...
template<typename A>
hll_sketch_alloc<A>::hll_sketch_alloc(uint8_t lg_config_k, target_hll_type tgt_type, bool start_full_size, const A& allocator) : hll_data() {
...
template<typename A>
bool hll_sketch_alloc<A>::serialize2(bool compact, unsigned char** outBytes, size_t* outSize) const {
  return sketch_impl->serialize2(compact, outBytes, outSize);
}
...
std::string hll_sketch_alloc<A>::type_as_string() const {
  switch (sketch_impl->getTgtHllType()) {
    ...
    default:
      THROW_RUNTIME_ERR("Sketch state error: Invalid target_hll_type");
  }
}
...
std::string hll_sketch_alloc<A>::mode_as_string() const {
  switch (sketch_impl->getCurMode()) {
    ...
    default:
      THROW_RUNTIME_ERR("Sketch state error: Invalid hll_mode");
  }
}
